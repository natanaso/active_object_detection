...
int
ejs_planner::get_EJS_action( double eps, int curr_vp,
					  	  ...
					  	  std::vector<double>::iterator curr_bel_end )
{
	...
	{
		double ejs = EJS( vp, oMap, curr_bel_start, curr_bel_end) / cMap[curr_vp][vp];
		if( ejs >= max_EJS )
		{
			max_EJS = ejs;
			best_act = vp;
...
void
ejs_planner::get_EJS_sequence( double eps, int curr_vp,
									  boost::multi_array<double,3> const& oMap,
									  boost::multi_array<double,2> const& cMap,
									  std::vector<double>::iterator curr_bel_start, 
								  	  std::vector<double>::iterator curr_bel_end,
								  	  std::vector<int> & vp_seq )
{
	...
	for( int vp = 0; vp < num_vp; ++vp)
		ejs_vp_vec.push_back( std::make_pair( 
				EJS( vp, oMap, curr_bel_start, curr_bel_end) / cMap[curr_vp][vp]
				, vp ) );
...
double
ejs_planner::EJS( int vp, boost::multi_array<double,3> const& oMap,
				 std::vector<double>::iterator curr_bel_start, 
				 std::vector<double>::iterator curr_bel_end )
{
...
double
ejs_planner::kl_div( boost::multi_array<double,1>::const_iterator p_start,
...
double
ejs_planner::kl_div( std::vector<double>::iterator p_start,
...
/*
int ejs_planner_main(int argc, char **argv)
{
	...
	double eps = 0.05;
	int best_act = ejs_planner::getEJSAction( eps, oMap, bel.begin(), bel.end() );
...
{
	return ejs_planner_main( argc, argv );
}
*/
