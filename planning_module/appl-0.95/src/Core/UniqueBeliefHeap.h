...
namespace momdp{
	...
	{
		BeliefVector_RowIndexPair(applSharedPointer<belief_vector>& bel, int row)
		{
			vector = bel;
			this->row = row;
		}

		applSharedPointer<belief_vector> vector;
		int row;
	};
	...
	public:
		...
		* @param row row index
		*/
		int addBeliefRowPair(applSharedPointer<belief_vector>& bel, int row);
		...
		*/
		int getBeliefRowIndex(applSharedPointer<belief_vector>& bel) const;
		...
		*/
		bool isStorageMaterial(applSharedPointer<belief_vector>& bel);
