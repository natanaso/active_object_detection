...
namespace momdp
{
	int UniqueBeliefHeap::addBeliefRowPair(applSharedPointer<belief_vector>& bel, int row)
	{
	...
	bool UniqueBeliefHeap::hasBelief(applSharedPointer<belief_vector>& bel)
	{
		int rowIndex = getBeliefRowIndex(bel);
		return (rowIndex >= 0);
	}

	int UniqueBeliefHeap::getBeliefRowIndex(applSharedPointer<belief_vector>& bel)const 
	{
		string newHash = bel->md5HashValue();
	...
	bool UniqueBeliefHeap::isStorageMaterial(applSharedPointer<belief_vector>& bel) {
