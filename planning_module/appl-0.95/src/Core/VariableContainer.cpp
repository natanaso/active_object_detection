...
applSharedPointer<SparseVector> VariableContainer::getInitialProb()
{
	DenseVector initialProb;
	initialProb.resize(this->size());

	applSharedPointer<SparseVector> result (new SparseVector());

	vector<vector<applSharedPointer<IVariableValue> > > valueListArray;
    ...
    {
        map<string, applSharedPointer<IVariableValue> > values;
		double prob = 1.0;
        for(int i = 0 ; i < valueIndex.size() ; i ++)
        {
            int subIndex = valueIndex[i];
            applSharedPointer<IVariableValue> curPart = valueListArray[i][subIndex];
...
int VariableContainer::indexOf(map<string, applSharedPointer<IVariableValue> >& input)
{
