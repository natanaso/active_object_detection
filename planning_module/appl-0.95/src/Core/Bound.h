...
namespace momdp 
{
	...
	{
	public:
		typedef void (*BackupCallback)(PointBasedAlgorithm *solver, BeliefTreeNode * node, applSharedPointer<T> backupResult) ;
	...
	protected:
		...
		void doNothing(BeliefTreeNode * node, applSharedPointer<T> backupResult)
		{
		}
	...
	public:
		...
		//virtual REAL_VALUE getValue(applSharedPointer<Belief>& belief) = 0;
		// Different type of Beliefs have no common interface, therefore, should not explicitly use it

		virtual applSharedPointer<T> backup(BeliefTreeNode * node) = 0;
