...
namespace momdp
{
	...
	*streamOut<<")" << endl;
    ...
    void SimulationEngine::setup(applSharedPointer<MOMDP> problem, applSharedPointer<AlphaVectorPolicy> policy, SolverParams * solverParams)
    {
        this->policy = policy;
    ...
    void SimulationEngine::performActionObs(belief_vector& outBelObs, int action, const BeliefWithState& belSt) const 
    {
	    ...
	    const applSharedPointer<SparseMatrix>  transMatX = problem->XTrans->getMatrix(action, belSt.sval); 
  ...
  {
        const applSharedPointer<SparseMatrix>  transMatY = problem->YTrans->getMatrix(action, belSt.sval, currObsState);
    ...
    void SimulationEngine::getPossibleObservations(belief_vector& possObs, int action, 	const BeliefWithState& belSt) const
    {
		...
		const applSharedPointer<SparseMatrix>  obsMat = problem->obsProb->getMatrix(action, belSt.sval);
    ...
    double SimulationEngine::getReward(const BeliefWithState& belst, int action)
    {
		...
		const applSharedPointer<SparseMatrix>  rewMat = problem->rewards->getMatrix(belst.sval);
    ...
    { 
        ...
        // actual system state
        applSharedPointer<BeliefWithState> actStateCompl (new BeliefWithState());
        applSharedPointer<BeliefWithState> actNewStateCompl (new BeliefWithState());

        // policy follower state
        // belief with state
        applSharedPointer<BeliefWithState> nextBelSt;
        applSharedPointer<BeliefWithState> currBelSt (new BeliefWithState());// for policy follower based on known x value
        ...
        if (problem->initialBeliefStval->sval == -1) // check if the initial starting state for X is fixed
        {
          ...
          const applSharedPointer<DenseVector>& startBeliefX = problem->initialBeliefX;
        ...
        applSharedPointer<SparseVector> startBeliefVec;
