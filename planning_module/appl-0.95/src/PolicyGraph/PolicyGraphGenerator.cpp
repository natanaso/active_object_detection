 ...
 */
    ...
    PolicyGraphGenerator::PolicyGraphGenerator(applSharedPointer<MOMDP> _problem, applSharedPointer<AlphaVectorPolicy> _policy, PolicyGraphParam _graphParam)
...
void PolicyGraphGenerator::generateNodesDot(ostream& output, BeliefTreeNode* node)
{
    ...
    applSharedPointer<belief_vector> curBelief = cacheRow->BELIEF;
...
void PolicyGraphGenerator::drawRootNodeDot(ostream& output, applSharedPointer<SparseVector> initialBeliefY, applSharedPointer<DenseVector> initialBeliefX, int bestAction)
{
    output << "root"; 
    output << " [label=\"" ;
...
void PolicyGraphGenerator::generateGraph(ostream& output)
{
    ...
    applSharedPointer<SparseVector> initialBeliefY = problem->getInitialBeliefY(0);
    ...
    applSharedPointer<DenseVector> initialBeliefX = problem->initialBeliefX;
	...
	FOR(Xn, problem->XStates->size()){
		...
		{
			applSharedPointer<BeliefWithState>  thisRootb_s = problem->beliefTransition->nextBelief(initialBeliefY, *initialBeliefX, initialBestAction, O, Xn);  
