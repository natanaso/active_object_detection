...
custom_Matrix *P;  /* Transition Probabilities */

custom_Matrix *R;  /* Observation Probabilities */

custom_Matrix Q;  /* Immediate values for state action pairs.  These are
...
void convertMatrices() {
	...
	*/
	...
	P = (custom_Matrix *) malloc( gNumActions * sizeof( *P ) );
	checkAllocatedPointer((void *) P );

	R = (custom_Matrix *) malloc( gNumActions * sizeof( *R ) );
