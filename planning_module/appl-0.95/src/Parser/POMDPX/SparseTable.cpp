...
applSharedPointer<SparseTable> SparseTable::removeUnmatchedCI(int cIIndex, int uIIndex) {
     ...
     applSharedPointer<SparseTable> removedcITable (new SparseTable(cIheader, uIheader, numCIValues, numUIValues));
...
applSharedPointer<SparseTable> SparseTable::joinHeader(SparseTable& A, SparseTable& B, int& numCommonIndexes){
     unsigned int pos;
     vector<string> indexes;
     //This is to handle the random ordering of the parents:
     indexes = A.findIntersectingCI(applSharedPointer<SparseTable>(&B));
     ...
     return applSharedPointer<SparseTable>(new SparseTable(cIheader, uIheader, numCIValues, numUIValues));
}

//very important part of the parser.
applSharedPointer<SparseTable> SparseTable::join(SparseTable& A, SparseTable& B, int whichFunction) {
     int numCommonIndexes;

     //join headers of A and B, at the same time heads that exist in both tables are moved to front
     applSharedPointer<SparseTable> C = SparseTable::joinHeader(A,B, numCommonIndexes);
...
vector<string> SparseTable::findIntersectingCI(applSharedPointer<SparseTable> st) {
     vector<string> index;
