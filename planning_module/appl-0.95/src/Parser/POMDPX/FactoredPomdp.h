...
typedef vector<vector<applSharedPointer<SparseMatrix> > > vvSparseMatrix;
...
namespace momdp {
    ...
    class FactoredPomdp {
	    ...
	    const set<string> getRewardFunctionCurrentTimeSliceVars(Function* rewardFunc) ;
            applSharedPointer<SparseTable> removeRedundantUIsFromReward(applSharedPointer<SparseTable> st);
            applSharedPointer<SparseTable> combineSimilarEntriesInReward(applSharedPointer<SparseTable> st);
            applSharedPointer<SparseTable> preprocessRewardTable(); 	//called by convertFast
            ...
            applSharedPointer<SparseTable>  finalRewardTable;
            applSharedPointer<SparseTable>  finalStateTable;
            applSharedPointer<SparseTable>  finalBeliefTable;
            applSharedPointer<SparseTable>  mergeTables(vector<Function>* functionList, int whichFunction, ofstream& debugfile, bool printDebugFile);

	    applSharedPointer<SparseTable>  mergeSparseTables(vector<applSharedPointer<SparseTable> > stList, int whichFunction, ofstream& debugfile, bool printDebugFile); 
	    ...
	    bool preprocessBeliefTablesDone;
            ...
            applSharedPointer<SparseTable> expandObsRewSparseTable(applSharedPointer<SparseTable> st, int whichFunction);
            ...
            void mapObservationsUIsToValue(applSharedPointer<SparseTable> st);
	    ...
	    bool getNextSVarValues(map<string, int> &curValues,int &stateNum);
            ...
            void mapFastIndexesToValues(applSharedPointer<SparseTable> st);
            void convertFast();
            ...
            applSharedPointer<SparseTable> reduceUnmatchedCIWithUI(applSharedPointer<SparseTable>  st, ofstream& debugfile, bool printDebugFile);
	    ...
	    void printSparseMatrix(string title, vector<vector<applSharedPointer<SparseMatrix> > > M, ofstream& debugfile);

            void resortFactoredStateTables(ofstream& debugfile, bool printDebugFile, const int MIXEDTYPE);
            void mapFactoredStatesToValue();
            void mapFactoredCIsToValue(applSharedPointer<SparseTable> st);
            void mapFactoredStateUIsToValue(applSharedPointer<SparseTable> st);
            void mapFactoredBeliefIndexesToValue(applSharedPointer<SparseTable> st);
            void convertFactored();
            void convertFactoredReparam();
            void convertFactoredStateTrans();
            void convertFactoredStateReparamTrans();
            void expandFactoredStateTable(applSharedPointer<SparseTable> sf);
            ...
            vector<vector<applSharedPointer<SparseMatrix> > > helperPreSparseMatrixToSparseMatrix(vector< vector<PreSparseMatrix> > precm);
            vector<vector<vector<applSharedPointer<SparseMatrix> > > > helperPreSparseMatrixToSparseMatrix(vector< vector<vector<PreSparseMatrix> > >precm);
