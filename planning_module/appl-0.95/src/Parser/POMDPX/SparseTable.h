...
class SparseTable : public MObject
{
	...
	void removeRedundant();
        ...
        vector<string> findIntersectingCI(applSharedPointer<SparseTable> st);
	...
	string getInfo();
        ...
        static applSharedPointer<SparseTable> join(SparseTable& A, SparseTable& B, int whichFunction);
        static SparseEntry mergeSparseEntry(SparseEntry& A, SparseEntry& B, int numCommonIndexes);
	
	static applSharedPointer<SparseTable> joinHeader(SparseTable& A, SparseTable& B, int& numCommonIndexes);
	

        int getCIPosition(string str);
        applSharedPointer<SparseTable> removeUnmatchedCI(int cIIndex, int uIIndex);
