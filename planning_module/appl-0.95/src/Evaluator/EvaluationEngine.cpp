...
namespace momdp
{
	...
	void EvaluationEngine::setup(applSharedPointer<MOMDP> problem, applSharedPointer<AlphaVectorPolicy> policy, BeliefForest* _beliefForest,vector <BeliefCache *> *_beliefCacheSet,  Sample *_sampleEngine, SolverParams * solverParams)
	{
		this->policy = policy;
  ...
  /*	void EvaluationEngine::performAction(belief_vector& outBelUnobs, belief_vector& outBelObs, int action, const BeliefWithState& belSt) const 
	{
		...
		const applSharedPointer<SparseMatrix>  transMatY = problem->XYTrans->getMatrix(action, belSt.sval);
  ...
  */
	...
	BeliefTreeNode* EvaluationEngine::searchNode(applSharedPointer<BeliefWithState>  belief)
	{
		applSharedPointer<belief_vector> s = belief->bvec;
	...
	{ 
		...
		// belief with state
		applSharedPointer<BeliefWithState> currBelSt = curNode->s;
		...
		{ 
			...
			applSharedPointer<obs_prob_vector> obsPoss = afterObsDataTupel->opv;
