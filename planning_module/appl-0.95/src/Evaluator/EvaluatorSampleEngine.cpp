...
namespace momdp
{
	...
	****************************************/
	...
	BeliefTreeNode* EvaluatorSampleEngine::getNode(applSharedPointer<BeliefWithState>& b_s) 
	{
		applSharedPointer<belief_vector> s = b_s->bvec;
	...
	void EvaluatorSampleEngine::expand(BeliefTreeNode& cn)
	{
		...
		applSharedPointer<BeliefWithState> sp; // state_vector sp;
		...
		for(Actions::iterator aIter =  problem->actions->begin() ; aIter != problem->actions->end(); aIter ++)
		{
			...
			((EvaluatorAfterActionDataTuple*)Qa.extraData)->spv = applSharedPointer<SparseVector>(new SparseVector());
			...
			for(States::iterator xIter =  problem->XStates->begin() ; xIter != problem->XStates->end(); xIter ++)
			{
				...
				if (sprob > OBS_IS_ZERO_EPS) 
				{
					...
					problem->getJointUnobsStateProbVector(*jspv, (applSharedPointer<BeliefWithState> )cn.s, a, Xn);
