...
REAL_VALUE AlphaPlanePoolSet::getValue(applSharedPointer<BeliefWithState>& bns)
{
...
applSharedPointer<AlphaPlane> AlphaPlanePoolSet::getValueAlpha(applSharedPointer<BeliefWithState>& bns)
{
	state_val sval = bns->sval;
	return set[sval]->getValueAlpha( bns->bvec);
}

applSharedPointer<AlphaPlane> AlphaPlanePoolSet::backup(BeliefTreeNode * node)
{
...
applSharedPointer<AlphaPlane> AlphaPlanePoolSet::getBestAlphaPlane(BeliefWithState& beliefandState)
{
	state_val sval = beliefandState.sval;
	applSharedPointer<AlphaPlane> bestAlp;
...
applSharedPointer<AlphaPlane> AlphaPlanePoolSet::getBestAlphaPlane(BeliefTreeNode& cn)
{
...
applSharedPointer<AlphaPlane> AlphaPlanePoolSet::getBestAlphaPlane1(BeliefWithState& beliefandState)
{
	state_val sval = beliefandState.sval;
	applSharedPointer<AlphaPlane>bestAlp;
...
void AlphaPlanePoolSet::writeToFile(const std::string& outFileName, string problemName) 
{
	...
	FOR (sval, set.size())
	{
		list<applSharedPointer<AlphaPlane> >::const_iterator pi = set[sval]->planes.begin();
...
void AlphaPlanePoolSet::writeToFile(const std::string& outFileName) 
{
	...
	list<applSharedPointer<AlphaPlane> >::const_iterator pi = set.back()->planes.begin();
