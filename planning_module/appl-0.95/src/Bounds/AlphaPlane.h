...
public:
		...
		//methods
		void copyFrom(applSharedPointer<alpha_vector> _alpha, int _action, state_val _sval);

	virtual ~AlphaPlane(void);

		
		applSharedPointer<alpha_vector> alpha;
		int action;
		state_val sval;
		...
		applSharedPointer<AlphaPlane> duplicate();
