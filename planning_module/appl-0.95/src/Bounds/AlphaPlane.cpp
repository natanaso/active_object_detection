...
applSharedPointer<AlphaPlane> AlphaPlane::duplicate()
{
	applSharedPointer<AlphaPlane> result ( new AlphaPlane());
	result->alpha = applSharedPointer<alpha_vector> (new alpha_vector());
...
void AlphaPlane::copyFrom(applSharedPointer<alpha_vector> _alpha, int _action, state_val _sval)
{
	this->alpha = _alpha;
	this->action = _action;
	this->sval = _sval;
