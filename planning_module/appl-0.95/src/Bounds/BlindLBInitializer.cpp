...
namespace momdp {
	...
	BlindLBInitializer::BlindLBInitializer(applSharedPointer<MOMDP> _pomdp, AlphaPlanePoolSet* _bound) {
		pomdp = (applSharedPointer<MOMDP>) _pomdp;
		bound = _bound;
	}
...
//				DEBUG_TRACE(diff.write(cout) << endl; );
			...
			#else
			applSharedPointer<AlphaPlane> plane = new AlphaPlane(al, a);
	...
	void BlindLBInitializer::initBlindWorstCase(std::vector<alpha_vector>& weakAlphas)
	{
		...
		{	
			...
			applSharedPointer<SparseMatrix>  rewardMatrix = pomdp->rewards->getMatrix(sval);
