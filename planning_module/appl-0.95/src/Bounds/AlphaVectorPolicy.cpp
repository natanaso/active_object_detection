...
AlphaVectorPolicy::AlphaVectorPolicy(applSharedPointer<MOMDP> problem)
{
...
{
	applSharedPointer<AlphaPlane> bestAlpha = alphaPlanePoolSet->set[b.sval]->getBestAlphaPlane1(b.bvec);
...
int AlphaVectorPolicy::getBestActionLookAhead(BeliefWithState& b, REAL_VALUE& maxValue)
{
	...
	applSharedPointer<BeliefWithState> nextb;
	obsState_prob_vector spv;  // outcome probability for values of observed state
	applSharedPointer<SparseVector> jspv (new SparseVector());
			...
			if (sprob > OBS_IS_ZERO_EPS) 
			{
				...
				FOR(o, opv.size()) 
				{
					...
					if (oprob > OBS_IS_ZERO_EPS) 
					{
						...
						applSharedPointer<AlphaPlane> bestAlpha = alphaPlanePoolSet->getBestAlphaPlane1(*nextb);
...
int AlphaVectorPolicy::getBestActionLookAhead(std::vector<belief_vector>& belYs, DenseVector& belX)
{
	...
	applSharedPointer<BeliefWithState> nextStB (new BeliefWithState());
	applSharedPointer<BeliefWithState> currStB (new BeliefWithState());
...
// SYL07282010 - modify function so that it follows RSS09 paper
int AlphaVectorPolicy::getBestActionLookAhead(applSharedPointer<belief_vector>& belY, DenseVector& belX)
{
	unsigned int bestAction  = 0;
	double bestValue, observationValue, actionValue;
	// observationValue is the summation over x'; actionValue is the summation over o; bestValue tracks the highest value among the actions, a.
...
int AlphaVectorPolicy::getBestActionLookAhead_alternative(std::vector<belief_vector>& b, DenseVector& belX)
{
	...
	applSharedPointer<BeliefWithState> currStB (new BeliefWithState());
	applSharedPointer<BeliefWithState> nextStB;
...
int AlphaVectorPolicy::getBestActionLookAhead_alternative(applSharedPointer<belief_vector>& b, DenseVector& belX)
{
...
int AlphaVectorPolicy::getBestAction(applSharedPointer<belief_vector>& b, DenseVector& belX)
{
	int maxAction;
	...
	FOR (xc, problem->XStates->size())
	{
		if (!(belX(xc) == 0))
		{
			...
			applSharedPointer<AlphaPlane> bestAlpha = alphaPlanePoolSet->set[xc]->getBestAlphaPlane1(b);;
...
int AlphaVectorPolicy::getBestAction(std::vector<belief_vector>& belYs, DenseVector& belX)
{
	...
	applSharedPointer<belief_vector> Bc (new belief_vector());
	...
	FOR (xc, problem->XStates->size())
	{
		if (!(belX(xc) == 0))
		{
			...
			applSharedPointer<AlphaPlane> bestAlpha = alphaPlanePoolSet->set[xc]->getBestAlphaPlane1(Bc);
			//applSharedPointer<AlphaPlane> bestAlpha = alphaPlanePoolSet->set[xc]->getBestAlphaPlane1(b);
