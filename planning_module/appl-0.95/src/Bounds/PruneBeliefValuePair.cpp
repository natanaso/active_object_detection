...
namespace momdp{
	...
	void PruneBeliefValuePair::prunePoints(void) 
	{
		...
		list< std::list<applSharedPointer<BeliefValuePair> >::iterator > erase_ptrs;

		LISTFOREACH_NOCONST(applSharedPointer<BeliefValuePair> , ptp,  bvPairPool->points) 
		{
			applSharedPointer<BeliefValuePair> try_pair = *ptp;
		...
		LISTFOREACH(list<applSharedPointer<BeliefValuePair> >::iterator, erase_ptr,  erase_ptrs) 
		{
			list<applSharedPointer<BeliefValuePair> >::iterator  x, xp1;
			x = xp1 = (*erase_ptr);
			xp1++;
