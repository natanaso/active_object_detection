...
REAL_VALUE BeliefValuePairPool::getValue(applSharedPointer<Belief>& b) 
{
	...
	for(list<applSharedPointer<BeliefValuePair> >::iterator iter = points.begin(); iter!=points.end(); iter ++) 
	{
...
applSharedPointer<BeliefValuePair> BeliefValuePairPool::addPoint(applSharedPointer<belief_vector>&  b, REAL_VALUE val) 
{
	...
	if (-1 == wc) 
	{
		applSharedPointer<BeliefValuePair>bvp = new BeliefValuePair();
...
int BeliefValuePairPool::whichCornerPoint(const applSharedPointer<belief_vector>&  b) const {
