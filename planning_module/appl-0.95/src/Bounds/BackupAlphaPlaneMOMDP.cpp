...
applSharedPointer<AlphaPlane> BackupAlphaPlaneMOMDP::backup(BeliefTreeNode* cn)
{
	applSharedPointer<AlphaPlane> newPlaneP (new AlphaPlane());
	...
	list<applSharedPointer<AlphaPlane> >* alphas = bound->dataTable->get(cn->cacheIndex.row).ALPHA_PLANES;
						...
						//FOR (o, QaXn.getNumOutcomes())
					{
						...
						if (NULL != e)
						{
							...
							*/	
							...
							applSharedPointer<AlphaPlane> tempAlpha = boundSet->getBestAlphaPlane(*(childNode));
...
{
	applSharedPointer<alpha_vector> betaA (new alpha_vector(problem->getBeliefSize()));
	applSharedPointer<alpha_vector> betaAXnO;
	// use pointer instead of creating a new alpha vector, Yanzhu
	alpha_vector tmp, tmp1, tmp2;

	bool defaultIsSet;
	applSharedPointer<BeliefWithState> dummy_stval (new BeliefWithState());

	
	state_val Xc = cn.s->sval;	// state value at current time step (ie X, not X')
	applSharedPointer<alpha_vector> defaultBetaAXnO;
	const BeliefTreeQEntry& Qa = cn.Q[a];
