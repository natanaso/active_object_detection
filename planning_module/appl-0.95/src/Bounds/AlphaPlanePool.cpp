...
applSharedPointer<AlphaPlane> AlphaPlanePool::getValueAlpha(applSharedPointer<Belief>& belief)
{
	return getBestAlphaPlane(belief, -1);
}

void AlphaPlanePool::addAlphaPlane(applSharedPointer<AlphaPlane> plane)
{
...
applSharedPointer<AlphaPlane> AlphaPlanePool::getBestAlphaPlane( applSharedPointer<belief_vector>& b, int index )  //  belief, belief index
{
	DEBUG_TRACE( cout << "getBestAlphaPlane b" << endl; );
	DEBUG_TRACE( b->write(cout) << endl; );

	double val,maxval = -99e+20;
	applSharedPointer<AlphaPlane> ret = NULL;
	...
	if(index != -1)
	{
		...
		list<applSharedPointer<AlphaPlane> >* alphas = dataTable->get(index).ALPHA_PLANES;
	...
	LISTFOREACH(applSharedPointer<AlphaPlane>, pr,  this->planes) 
	{
		applSharedPointer<AlphaPlane> al = *pr;
	...
	if(index != -1)
	{
		if(ret!= NULL)
		{
			...
			if(alphas->size()>0)
			{
				applSharedPointer<AlphaPlane> frontAlpha = alphas->front();
...
applSharedPointer<AlphaPlane> AlphaPlanePool::getBestAlphaPlane( BeliefTreeNode& cn) 
{
	applSharedPointer<belief_vector>  b = cn.s->bvec;
	int index = cn.cacheIndex.row;
	return getBestAlphaPlane(b, index ) ;
}

applSharedPointer<AlphaPlane> AlphaPlanePool::getBestAlphaPlane( applSharedPointer<belief_vector>& b) 
{
...
applSharedPointer<AlphaPlane> AlphaPlanePool::getBestAlphaPlane1( applSharedPointer<belief_vector>& b) 
{
	double val, maxval = -99e+20;
	applSharedPointer<AlphaPlane> ret = NULL;
...
REAL_VALUE AlphaPlanePool::getValue(applSharedPointer<Belief>& belief)
{
	double v = inner_prod( *getBestAlphaPlane(belief)->alpha, *belief );
	return v;
}
double AlphaPlanePool::getValue( applSharedPointer<belief_vector>& belief, applSharedPointer<AlphaPlane>* resultBestAlpha)
{
	applSharedPointer<AlphaPlane>bestAlpha = getBestAlphaPlane(belief);
