...
namespace momdp
{
	...
	**************************************************************/
	...
	bool PruneAlphaPlane::isMax(applSharedPointer<AlphaPlane> alphaPlane)
	{
	...
	{
		list<applSharedPointer<AlphaPlane> > next_planes;
		next_planes.clear();

		//iterate planes, only retain the ones which has certificates or has
		//	been useful at corners
		LISTFOREACH (applSharedPointer<AlphaPlane>, try_pair, alphaPlanePool->planes) 
		{
			applSharedPointer<AlphaPlane> try_alpha = *try_pair;
...
cout << "Last alpha in the pool (not deleting) for sval: " << alphaPlanePool->sval << endl;
	...
	void PruneAlphaPlane::Sanity()
	{
		...
		for(int i = 0 ; i < alphaPlanePool->beliefCache->currentRowCount ; i ++)
		{
			...
			applSharedPointer<AlphaPlane> maxAlpha = NULL;
			const applSharedPointer<belief_vector>  b = alphaPlanePool->beliefCache ->getRow(i)->BELIEF;
...
unsigned int planeCount = 0;
		...
		if(alphaPlanePool->planes.size()>0)
		{
			...
			vector <applSharedPointer<AlphaPlane> > cornerPlanes;
			//init corners
...
for (int i = 0; i<beliefSize; i++)
{
	...
	cout << " : " << cornerPlanes.at(i) << endl;
			...
			LISTFOREACH(applSharedPointer<AlphaPlane>,  plane,  alphaPlanePool->planes) {
...
for (int i = 0; i<beliefSize; i++)
{
	...
	cout << " : " << cornerPlanes.at(i) << endl;
			...
			{
				applSharedPointer<AlphaPlane> plane = cornerPlanes.at(i);
...
cout << "alphaPlanePool->planes.size() !> 0 !!!" <<endl;
 ...
 cout << " planeCount : " << planeCount << endl;
	...
	void PruneAlphaPlane::updateCorners(applSharedPointer<AlphaPlane> plane, std::vector<double, std::allocator<double> >* values, std::vector<applSharedPointer<AlphaPlane>,  std::allocator<applSharedPointer<AlphaPlane> > >* indices)
	{
		applSharedPointer<alpha_vector> alpha = plane->alpha;
	...
	void PruneAlphaPlane::updateMax(applSharedPointer<AlphaPlane> plane, double delta, int timeStamp)
	{
		...
		FOREACH(AlphaPlaneMaxMeta* , entry, planeTuple->maxMeta)
		{
			...
			{
				const applSharedPointer<belief_vector>  b = alphaPlanePool->beliefCache ->getRow(i)->BELIEF;
				//const applSharedPointer<belief_vector>  b = GlobalResource::getInstance()->beliefCache ->getRow(i)->BELIEF;
	...
	*/
	...
	void PruneAlphaPlane::updateCerts(applSharedPointer<AlphaPlane> plane, double delta, int timeStamp)
	{
		...
		for(int i=0; i<(int)planeTuple->certifiedBeliefs.size(); i++)
		{
			...
			applSharedPointer<belief_vector>  b = n->s->bvec;
			...
			if(n->count > 0)
			{
				...
				maxValue = value;
				dominated = false;
				applSharedPointer<AlphaPlane> maxPlane = NULL;//may cause trouble later
				LISTFOREACH(applSharedPointer<AlphaPlane>,  try_pair,  alphaPlanePool->planes) 
				{
					applSharedPointer<AlphaPlane> try_alpha = * try_pair;
	...
	int PruneAlphaPlane::countCerts(void)
	{
		...
		LISTFOREACH(applSharedPointer<AlphaPlane>,  plane,  alphaPlanePool->planes) 
		{
			applSharedPointer<AlphaPlane> alpha = *plane;
	...
	{
		list<applSharedPointer<AlphaPlane> > next_planes;
		list<list<applSharedPointer<AlphaPlane> >::iterator> erase_ptrs;
		bool try_dominates_in, in_dominates_try;

		LISTFOREACH_NOCONST(applSharedPointer<AlphaPlane>,  try_pair,  alphaPlanePool->planes) 
		{
			applSharedPointer<AlphaPlane> try_alpha = *try_pair;
			erase_ptrs.clear();
			LISTFOREACH_NOCONST(applSharedPointer<AlphaPlane>,  in_pair,  next_planes) 
			{
				applSharedPointer<AlphaPlane> in_alpha = *in_pair;
			...
			LISTFOREACH_NOCONST(list<applSharedPointer<AlphaPlane> >::iterator,  erase_ptr,  erase_ptrs) 
			{
				list<applSharedPointer<AlphaPlane> >::iterator  x, xp1;
				applSharedPointer<AlphaPlane> testDeleteAlphaPlane = (**erase_ptr);
