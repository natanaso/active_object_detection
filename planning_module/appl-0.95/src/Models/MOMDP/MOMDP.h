...
namespace momdp 
{
	...
	private:
		...
		virtual void deleteMatrixVector(vector<applSharedPointer<SparseMatrix> > *m);

	public:
        applSharedPointer<SparseVector> initialBeliefY;
        vector<applSharedPointer<SparseVector> > initialBeliefYByX;

		Cache<applSharedPointer<SparseMatrix> > cache;
				
		MOMDP(void); // default constructor

		
		static applSharedPointer<MOMDP> convertMOMDPFromPOMDP(POMDP* pomdpProblem);
		static applSharedPointer<MOMDP> convertMOMDPFromPOMDPX(FactoredPomdp* factoredPomdp, bool assumeUnknownFlag,unsigned int probType);
		...
		REAL_VALUE discount;
                ...
                applSharedPointer<BeliefWithState>  initialBeliefStval;
		applSharedPointer<DenseVector> initialBeliefX;
		...
		vector<applSharedPointer<SparseMatrix> > *pomdpT, *pomdpTtr, *pomdpO;
		...
		inline applSharedPointer<SparseVector> getInitialBeliefY(int obsState)
		{
		...
		virtual obsState_prob_vector& getObsStateProbVector(obsState_prob_vector& result, applSharedPointer<belief_vector>& belY, DenseVector& belX, int a);
		virtual obsState_prob_vector& getObsStateProbVector(obsState_prob_vector& result, BeliefWithState& b, int a);
		// Xn: next X
		virtual SparseVector& getJointUnobsStateProbVector(SparseVector& result, applSharedPointer<BeliefWithState> b, int a, int Xn);
		...
		virtual void getObsProbVector(obs_prob_vector& result, applSharedPointer<belief_vector>& belY, obsState_prob_vector& belX, int a, int Xn);
