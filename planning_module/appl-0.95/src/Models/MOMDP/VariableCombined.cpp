...
void VariableCombined::addSubVar(applSharedPointer<IVariable> subVar)
{
	subVariables.push_back(subVar);
}

applSharedPointer<IVariable> VariableCombined::getSubVar(int index)
{
...
void VariableCombined::seal()
{
	...
	{
		applSharedPointer<IVariable> subVariable = subVariables[i];
