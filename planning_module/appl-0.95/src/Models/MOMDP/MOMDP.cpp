                     ...
                     hasIntraslice(false)
{
    ...
    beliefTransition->problem =appl_dynamic_pointer_cast<MObject>( applSharedPointer<MOMDP>(this));
    discount = 0.95;
...
void MOMDP::deleteMatrixVector(vector<applSharedPointer<SparseMatrix> > *m)
{
    /*FOREACH_NOCONST(applSharedPointer<SparseMatrix> , ppMatrix, *m)
      {
      applSharedPointer<SparseMatrix>  curMatrix = *ppMatrix;
      if(curMatrix !=NULL)
      {
      delete curMatrix;
      }
...
SparseVector&  MOMDP::getJointUnobsStateProbVector(SparseVector& result, applSharedPointer<BeliefWithState> b, int a, int Xn) 
{
    int Xc = b->sval; // currrent value for observed state variable
...
obsState_prob_vector& MOMDP::getObsStateProbVector(obsState_prob_vector& result, applSharedPointer<belief_vector>& belY, DenseVector& belX, int a)
{
    DenseVector Bc; 
    copy(Bc, *belY);
...
void MOMDP::getObsProbVector(obs_prob_vector& result, applSharedPointer<belief_vector>& belY, obsState_prob_vector& belX, int a, int Xn) 
{
    DenseVector Bc; 
    copy(Bc, *belY);
...
applSharedPointer<MOMDP> MOMDP::convertMOMDPFromPOMDP(POMDP* pomdpProblem)
{
    applSharedPointer<MOMDP> result (new MOMDP());
    ...
    /* result->pomdpR = &(pomdpProblem->R);
       ...
       result->pomdpT = new vector<applSharedPointer<SparseMatrix> >();
       result->pomdpTtr = new vector<applSharedPointer<SparseMatrix> >();
       result->pomdpO = new vector<applSharedPointer<SparseMatrix> >();
...
applSharedPointer<MOMDP> MOMDP::convertMOMDPFromPOMDPX(FactoredPomdp* factoredPomdp, bool assumeUnknownFlag,unsigned int probType) 
{
    applSharedPointer<MOMDP> result (new MOMDP());
    ...
    // R[x] (y,a)
    // 	vector<applSharedPointer<SparseMatrix> > R;
    // 	// R(s,a)
    // 	//T[a](s,s'), Ttr[a](s',s), O[a](s',o)
    // 	vector<applSharedPointer<SparseMatrix> > *pomdpOtr;
    // 
    // 	// TX[a][x](y,x'), TXtr[a][x](x',y), TY[a][x](y,y'), TYtr[a][x](y',y)
    // 	// O[a][x'](y',o), Otr[a][x'](o,y')
    // 	vector<vector<applSharedPointer<SparseMatrix> > > TX, TXtr, TY, TYtr, O, Otr;
	...
	{ // this option only makes sense for mixed observable cases
	    ...
	    result->pomdpT = new  std::vector<applSharedPointer<SparseMatrix> >;
	    result->pomdpTtr = new std::vector<applSharedPointer<SparseMatrix> >;
	    result->pomdpO = new std::vector<applSharedPointer<SparseMatrix> >;
	    // pomdpOtr = new std::vector<applSharedPointer<SparseMatrix> >;  // SYL040909 commented out
	...
	FOR(stateidx, numStates){
	    ...
	    result->rewards->matrix[stateidx] = applSharedPointer<SparseMatrix>(new SparseMatrix());
	...
	FOR (a, numActions) {
	    ...
	    FOR(stateidx, numStates){
		XYTrans->matrix[a][stateidx] = applSharedPointer<SparseMatrix>(new SparseMatrix());
		copy(*XYTrans->matrix[a][stateidx], Tdummy);
		XYTrans->matrixTr[a][stateidx] = applSharedPointer<SparseMatrix>(new SparseMatrix());
		...
		result->obsProb->matrix[a][stateidx] = applSharedPointer<SparseMatrix>(new SparseMatrix());
		copy(*result->obsProb->matrix[a][stateidx], ObsMatrix);
		kmatrix_transpose_in_place (ObsMatrix);
		result->obsProb->matrixTr[a][stateidx] = applSharedPointer<SparseMatrix>(new SparseMatrix());
		...
		result->XTrans->matrix[a][stateidx] = applSharedPointer<SparseMatrix>(new SparseMatrix());
...
bool MOMDP::getIsTerminalState(BeliefWithState &b)
{
    ...
    applSharedPointer<belief_vector> s = b.bvec;
