...
#define CHECK_INTERVAL 50
	...
	public:
		...
		SARSOP(applSharedPointer<MOMDP> problem, SolverParams * solverParams);
		virtual ~SARSOP(void);
		...
		virtual void solve(applSharedPointer<MOMDP> problem);
		...
		static void onLowerBoundBackup (PointBasedAlgorithm *solver, BeliefTreeNode * node, applSharedPointer<AlphaPlane> backupResult)
		{
			// updating certs, etc
		}

		static void onUpperBoundBackup (PointBasedAlgorithm *solver, BeliefTreeNode * node, applSharedPointer<BeliefValuePair> backupResult)
		{
		}

		static void onGetNode(PointBasedAlgorithm *solver, BeliefTreeNode* node, applSharedPointer<BeliefWithState>& belief)
		{
			...
			list<applSharedPointer<AlphaPlane> >* alphas = new list<applSharedPointer<AlphaPlane> >();
			sarsopSolver->lowerBoundSet->set[stateidx]->dataTable->set(row).ALPHA_PLANES= alphas;

			// TODO:: fix it
			applSharedPointer<AlphaPlane>alpha = sarsopSolver->lowerBoundSet->getValueAlpha(belief);
		...
		void initialize(applSharedPointer<MOMDP> problem);
		void initSampleEngine(applSharedPointer<MOMDP> problem);
		void initializeUpperBound(applSharedPointer<MOMDP> problem);
		void initializeLowerBound(applSharedPointer<MOMDP> problem);
		void initializeBounds(double _targetPrecision);

		// Alpha Vector Related
		static bool hasMaxMetaAt(applSharedPointer<AlphaPlane>alpha, int index)
		{
