...
namespace momdp
{
	...
	{
	public:
		typedef void (*GetNodeCallback)(PointBasedAlgorithm *solver, BeliefTreeNode *node, applSharedPointer<BeliefWithState>& belief) ;
	...
	public:
		...
		virtual void setup(PointBasedAlgorithm* _solver, applSharedPointer<MOMDP> _problem, vector <BeliefCache *> *_beliefCacheSet, BeliefForest* _beliefForest)
		{
			solver = _solver;
			problem = _problem;
		...
		//XXX ???
		virtual BeliefTreeNode* getNode(applSharedPointer<BeliefWithState>& s); // modified for factored, prevly, BeliefTreeNode* getNode(const state_vector& s);
